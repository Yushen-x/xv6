...
struct spinlock pgreflock; // 用于 pageref 数组的锁
int pageref[PGREF_MAX_ENTRIES]; // 每个物理页的引用计数
...
kfree(void *pa)
{
  ...
  acquire(&pgreflock);
  if(--PA2PGREF(pa) <= 0) { // 引用计数减1，如果小于等于0则释放
...
kcopy_n_deref(void *pa)
{
  ...
  // 因为我们仍然持有 pgreflock，所以可以直接在这里设置新页的引用计数
  PA2PGREF(newpa) = 1;
